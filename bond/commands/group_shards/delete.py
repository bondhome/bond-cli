...
class GroupShardDeleteCommand(object):
    subcmd = "delete"
    help = "Delete Bond's group shards."
    arguments = {
    ...
    def run(self, args):
        ...
        if args.all:
            if (
                args.force
                or input(f"Delete all group shards from {bond_id}? [N/y] ").lower() == "y"
            ):
                self.delete_all_groups(bond_id)
        elif args.group_ids:
            if (
                args.force
                or input(f"Delete group shard(s) {', '.join(args.group_ids)}? [N/y] ").lower() == "y"
            ):
                for group_id in args.group_ids:
                    bond.proto.delete(bond_id, topic=f"groups/{group_id}")
                    print(f"{group_id} group shard deleted.")
        else:
